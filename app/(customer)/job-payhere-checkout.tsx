...
export default function JobPayhereCheckoutScreen() {
  ...
  const workerName = typeof params.workerName === "string" ? params.workerName : "the professional";
    ...
    (async () => {
      try {
        const res = await apiCall(
          ...
          "POST",
          { orderId: `JOBPAY-${jobId}`, amount, currency: "LKR" },
          token,
        );
        ...
        const email = user?.email || "customer@clickfix.app";
        ...
        setHtmlContent(buildPayhereHtml({
          ...
          order_id: `JOBPAY-${jobId}`,
          items: `Service payment for job ${jobId}`,
          currency,
  ...
  const handlePaymentSuccess = async () => {
    try {
      await apiCall(api.jobs.pay(jobId), "PUT", { method: "online" }, token);
    } catch {
      // Confirming here is best-effort — the PayHere server notify webhook
      // also marks the job paid, so a transient failure isn't fatal.
    }

    Alert.alert("Payment Successful", `Your payment to ${workerName} is complete!`, [
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          ...
          <Text style={styles.headerSub}>
            {workerName} · {parseFloat(amount).toFixed(2)} LKR
          </Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
