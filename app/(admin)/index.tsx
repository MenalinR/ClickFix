...
export default function AdminDashboard() {
    ...
    const activeBookings = bookings?.filter(b => b.status === 'pending' || b.status === 'confirmed').length || 0;
    const completedBookings = bookings?.filter(b => b.status === 'completed').length || 0;

    const stats = [
        { icon: 'people', label: 'Total Workers', value: totalWorkers, color: Colors.primary },
        { icon: 'calendar', label: 'Total Bookings', value: totalBookings, color: Colors.accent },
        { icon: 'time', label: 'Active Bookings', value: activeBookings, color: '#FF6B6B' },
        { icon: 'checkmark-circle', label: 'Completed', value: completedBookings, color: '#4CAF50' },
    ];

    const recentActivity = [
        { id: 1, type: 'worker', text: 'New worker registered', time: '2 hours ago' },
        { id: 2, type: 'booking', text: 'Booking completed', time: '4 hours ago' },
        { id: 3, type: 'user', text: 'New customer registered', time: '6 hours ago' },
    ];
                    ...
                    {recentActivity.map((activity) => (
                        <View key={activity.id} style={styles.activityItem}>
                            <View style={styles.activityIcon}>
                                <Ionicons 
                                    name={activity.type === 'worker' ? 'person' : activity.type === 'booking' ? 'calendar' : 'people'} 
                                    size={20} 
