...
function useHardwareRequests() {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      const response = await fetch(
        "http://localhost:5000/api/hardware/requests",
        {
          headers: {
...
// Update if needed
db.hardwareitems.updateMany({ inStock: false }, { $set: { inStock: true } });
```
...
const items = await HardwareItem.find(query).skip(skip).limit(limit);
```
